...
import { i18n, formatMoney, formatDate } from '@ecomplus/utils';
...
export default {
    ...
    computed: {
        ...
        isSubscriptionDoppel() {
            return (
                this.orderBody &&
                this.orderBody.transactions &&
                this.orderBody.transactions.length &&
                this.orderBody.transactions.some(({ type }) => type === 'recurrence') &&
        ...
        orderNumber() {
            return (
                ...
                window.storefrontApp.router.currentRoute.params &&
                window.storefrontApp.router.currentRoute.params.number
            );
        },
        ...
        orderId() {
            return (
                ...
                window.storefrontApp.router.currentRoute &&
                window.storefrontApp.router.currentRoute.params &&
                window.storefrontApp.router.currentRoute.params.id
            );
        },
        ...
        financialStatus() {
            ...
            if (localOrder.payments_history) {
                ...
                localOrder.payments_history.forEach((record) => {
                    if (
                        record &&
                        (!transaction || !record.transaction_id || record.transaction_id === transaction._id) &&
                        (!statusRecord || !record.date_time || new Date(record.date_time).getTime() >= new Date(statusRecord.date_time).getTime())
                    ) {
                        statusRecord = record;
                    }
                });
        ...
        isSubscription() {
            return this.localOrder.transactions && this.localOrder.transactions.find(({ type }) => type === 'recurrence');
        },
    ...
    methods: {
        ...
        saveCustomerOrder() {
            ...
            if (!this.skipCustomerUpdate && localOrder.number && ecomPassport.checkAuthorization()) {
                ...
                ecomPassport.requestApi('/me.json').then(({ data }) => {
        ...
        toggle() {
            ...
            const data =
                this.localOrder.status !== 'cancelled'
                    ? {
                          status: 'cancelled',
            ...
            ecomPassport
                .requestApi(`/orders/${this.order._id}.json`, 'patch', data)
                .then(() => {
                    this.localOrder = {
                        ...this.localOrder,
                        ...data
        ...
        receiveDoppila: {
            handler(current, old) {
                ...
                if (current && current !== old && old !== null) {
                    ...
                    window.axios
                        .post(`https://sistema.doppelverso.com.br/ecom/doppila-or-box/${this.orderNumber}`, {
                            choice: current
                        })
                        .then((res) => {
        ...
        sessionChoice: {
            handler(current, old) {
                if (current && this.orderId && !window.sessionStorage.getItem('sent_metafield')) {
                    window.ecomPassport
                        .requestApi(`/orders/${this.orderId}/metafields.json`, 'POST', {
        ...
        size: {
            handler(current, old) {
                ...
                if (current && current !== old && this.sizes[current] && this.defaultValue !== this.sizes[current]) {
                    ...
                    window.axios
                        .post(`https://sistema.doppelverso.com.br/ecom/box-tshirt-choice/${this.orderNumber}`, {
    ...
    created() {
        if (this.order._id) {
            ...
            if (!this.skipDataLoad) {
                ...
                const update = () => {
                    const request = this.ecomPassport.checkAuthorization() ? this.ecomPassport.requestApi(url) : store({ url });
                    return request
                        .then(({ data }) => {
                            this.localOrder = {
                                ...this.localOrder,
        ...
        if (this.validThru) {
            ...
            if (validDate.getTime() > now) {
                ...
                const getRemainingTime = () => {
                    ...
                    return (days > 0 ? `${formatTime(days)} ${i19days} - ` : '') + `${formatTime(hours)}:${formatTime(minutes)}:${formatTime(seconds)}`;
                };
